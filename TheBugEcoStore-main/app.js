...
function renderCategoryFilters() {
  ...
  el.innerHTML = CATEGORIES.map(c => `
    <button
      ...
      } px-5 py-2 rounded-full border border-leaf-200">
      ${c}
    </button>
  `).join("");
}
...
function renderProducts() {
  ...
  grid.innerHTML = list.map(p => `
    <div class="bg-gradient-to-br from-leaf-50 to-leaf-200 rounded-xl shadow-lg border border-leaf-200 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer"
